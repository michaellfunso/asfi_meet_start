...
const uploadRecordings = (req, res) => {
  ...
  uploads(req, res, async function (err) {
    ...
    setTimeout(async () => {
      try {
        ...
        db.query("INSERT INTO recordings SET ?", [{ file_url: cloudinaryUrl, meeting_title:meetingId }], (err, result) => {
            if (err) {
                console.error("❌ Database Error:", err);
                return res.json({ error: "error", status: "Database error" });
            }else{
                 // ✅ Send response
